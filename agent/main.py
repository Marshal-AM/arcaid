        ...
        if market_id and market_address:
            ...
            if insert_to_db and supabase:
                try:
                    ...
                    # Prepare market data
                    # Note: eligible_ngo_ids is stored as text[] array
                    ...
                    market_data = {
                        ...
                        "eligible_ngo_ids": [ngo_id],  # Array of NGO IDs as hex strings (text[])
                        "state": "OPEN"
                    }
                    ...
                    # Check if it's a type format error
                    if "uuid" in error_msg.lower() or "22P02" in error_msg:
                        logger.error("⚠️  Database column 'eligible_ngo_ids' appears to have incorrect type.")
                        logger.error("   Blockchain NGO IDs are hex strings (bytes32), stored as text[].")
                        logger.error("   Please ensure the column type is text[] in Supabase.")
...
async def search_disasters():
    ...
    try:
        ...
        # Prepare the prompt
        prompt = f"""Search for ONE recent natural disaster or major catastrophe that occurred between {start_date_str} and {today_str} (today) in the world. 
...
async def verify_ngo_claim(request: VerifyRequest):
    ...
    try:
        ...
        prompt = f"""You are a fact-checking assistant verifying a disaster relief claim. Use web search to find reliable evidence and determine the truthfulness of the claim.

CLAIM TO VERIFY: "{request.question}"

VERIFICATION INSTRUCTIONS:
1. Search for reliable sources (news articles, official NGO reports, government statements, verified social media posts)
2. Evaluate the evidence found:
   - If multiple reliable sources confirm the claim → outcome=1 (YES/Confirmed)
   - If reliable sources refute the claim or show it's false → outcome=2 (NO/Refuted)
   - If insufficient evidence exists or the claim cannot be verified → outcome=3 (INVALID/Cannot Verify)
3. Set confidence based on evidence quality:
   - Strong evidence from multiple authoritative sources: 8500-10000 (85-100%)
   - Moderate evidence from reliable sources: 6500-8499 (65-84%)
   - Weak or conflicting evidence: 5000-6499 (50-64%)
   - Insufficient evidence: 3000-4999 (30-49%)

You must respond with ONLY a JSON object in this exact format:

{{
    "outcome": <1, 2, or 3>,
    "confidence": <integer between 3000 and 10000>,
    "evidence_string": "concise summary of evidence found, including key sources and findings"
}}

Where:
- outcome: 1 (Yes/Confirmed), 2 (No/Refuted), or 3 (Invalid/Cannot Verify)
- confidence: Integer representing confidence percentage (e.g., 9500 = 95%, 6500 = 65%)
- evidence_string: Brief summary citing specific sources and evidence (e.g., "Reuters and Red Cross official reports confirm 120,000 people received aid in Assam floods" or "No evidence found in official sources" or "Conflicting reports from different sources")
